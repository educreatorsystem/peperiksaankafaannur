...
function doGet() { return HtmlService.createHtmlOutputFromFile('Index').setTitle('Analisis Peperiksaan KAFA'); }
...
function setup_() {
  ...
  const user=p.getProperty('GURU_USERNAME'), pass=p.getProperty('GURU_PASSWORD');
  if(!user || !pass || pass.length<10) throw Error('Tetapkan GURU_USERNAME dan GURU_PASSWORD (minimum 10 aksara) dalam Script Properties.');
  const salt=Utilities.getUuid();
  p.setProperties({GURU_SALT:salt,GURU_HASH:hash_(salt+pass)});p.deleteProperty('GURU_PASSWORD');
...
function login(role,user,password) {
 const cache=CacheService.getScriptCache(),key='attempt:'+hash_(String(user)),n=Number(cache.get(key)||0);if(n>=8)throw Error('Terlalu banyak cubaan. Cuba selepas 10 minit.');cache.put(key,String(n+1),600);
 let s;
 if(role==='guru'){const p=PropertiesService.getScriptProperties();if(!p.getProperty('GURU_HASH'))throw Error('Pemilik perlu menjalankan setup_ dahulu.');if(user!==p.getProperty('GURU_USERNAME')||hash_(p.getProperty('GURU_SALT')+String(password))!==p.getProperty('GURU_HASH'))throw Error('Maklumat log masuk tidak sah.');s={role:'guru'};}
